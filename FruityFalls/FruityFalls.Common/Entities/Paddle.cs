...
namespace FruityFalls.Entities
{
	public class Paddle : CCNode
	{
        ...
        const float maxRotation = 60; // in degrees
		...
		Vine leftVine;
		Vine rightVine;
        private float paddleXVerlocity;
		...
		{
            // New code change image of the Paddle with the time
            Random rand1 = new Random();
            int intRan1 = rand1.Next();
            if (intRan1 % 2 == 0)
            {
                graphic = new CCSprite("monkey.png");
                graphic.IsAntialiased = false;
            ...
            {
                graphic = new CCSprite("monkey2.png");
                graphic.IsAntialiased = false;
                // offset it so the monkey's paddle lines up with the collision:
                graphic.PositionY = -24;
                this.AddChild(graphic);
            ...
            //new code
            //paddleXVerlocity += frameTimeInSeconds * GameCoefficients.gravityXacer;
            //this.PositionX += paddleXVerlocity * frameTimeInSeconds;
        ...
        public float Radius
        {
            get
            {
                return GameCoefficients.PaddleRadius;
            }
        }
